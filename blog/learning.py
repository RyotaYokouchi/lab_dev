...
# 加工後の画像ディレクトリのパス
train_dir_path = "./faces/augumente_faces/"
# テストディレクトリのパス
...
def data_augumente_pre():
    ...
    for name in original_dir:
        ...
        # 加工した画像を置く場所を確保
        os.makedirs(train_dir_path + name, exist_ok=True)
...
def data_augumente(face_img, name):
    ...
    cv2.imwrite(train_dir_path + name + "/" + name + "_img_rotate_left.jpg", img_rotate_left)
    ...
    cv2.imwrite(train_dir_path + name + "/" + name + "_img_rotate_center.jpg", img_rotate_center)
    ...
    cv2.imwrite(train_dir_path + name + "/" + name + "_img_rotate_rigt.jpg", img_rotate_right)
...
def gaussian_blur(face_img, rotate_value, name):
    ...
    # 書き込み
    cv2.imwrite(train_dir_path + name + file_name, img_filter)
...
def threshold(face_img, rotate_value, name):
    ...
    # 書き込み
    cv2.imwrite(train_dir_path + name + file_name, img_thr)
...
def reverse(face_img, rotate_value, name):
    ...
    # 書き込み
    cv2.imwrite(train_dir_path + name + file_name, img_flip)
...
def random_sort():
    ...
    for name in names:
        ...
        # 水増しした画像データの一覧取得
        imgs = os.listdir(train_dir_path + name)
        # imgsの数を元に、その中の２割の番号を取得
        random_numbers = random.sample(range(len(imgs)), k=round(len(imgs)*random_v))

        # ランダムに選んだ番号を元に画像を移動
        for random_number in random_numbers:
            shutil.move(train_dir_path + name + "/" + imgs[random_number], test_dir_path + name)
